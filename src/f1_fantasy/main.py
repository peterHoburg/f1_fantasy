...
class Driver:
    ...
    def _compute_qualifying_points(self):
        if self.qualifying_position is not None and self.qualifying_position <= 10:
            self.points += qualifying_place_points[self.qualifying_position - 1]
    ...
    def _compute_race_points(self):
        if self.race_position is not None and self.race_position <= 10:
            self.points += race_place_points[self.race_position - 1]
            if self.fastest_lap:
                self.points += 10
    ...
    def _compute_race_position_points(self):
        ...
        if position_delta > 0:
            self.points += position_delta * 2
        elif position_delta < 0:
            self.points += position_delta
    ...
    def compute_points(self):
        for driver in self.drivers:
            ...
            if driver.driver_of_the_day is True:
                self.points -= 10  # constructors do not get driver of the day bonuses
